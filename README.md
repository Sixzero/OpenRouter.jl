...
their own table via `COST_DISCOUNT_FN`, a resolver that takes the `ProviderEndpoint`
and returns a fraction off (match on `model_name`/`provider_name`, which are more
stable than the per-route `name`):

```julia
OpenRouter.COST_DISCOUNT_FN[] = ep -> occursin(r"opus 4\.7"i, ep.model_name) ? 0.5 : 0.0
```
